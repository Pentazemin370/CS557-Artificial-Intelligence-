...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for gp in lgpd:
           ...
           # bestest = max(gp)
           #print "bestest:" ,bestest
            #print "best:", best
            #print gp
        #if the ghost is closer than any of our legal actions, then just stay put
        choice = self.distancer.getDistance(pacmanPosition,loudest)
        bestAction = Directions.STOP
        #if not, pick the one that moves us towards it
        for move in legal:
            nextPos = pacmanPosition
            if move=="East":
                x,y = nextPos
                x += 1
                nextPos = x,y
            ...
            if move == "South":
                ...
                y -= 1
                nextPos = x, y
            nextDist = self.distancer.getDistance(nextPos,loudest)
            if nextDist<choice:
                choice = nextDist;
                bestAction = move;
